...
class PotentialToRenterChart extends StatefulWidget {
  @override
  _PotentialToRenterChartState createState() => _PotentialToRenterChartState();
}

class _PotentialToRenterChartState extends State<PotentialToRenterChart> {
  ReportPotentialToRenterController controller = Get.find();
              ...
              series: <LineSeries<SalesData, String>>[
                LineSeries<SalesData, String>(
                  ...
                  dataSource: <SalesData>[
                    ...List.generate(
                      (controller.reportPotentialToRenter.value.charts ?? [])
                          .length,
                      (index) => SalesData(
                          controller.reportPotentialToRenter.value.typeChart ==
                                  "month"
                              ? controller.listMonth[index]
                              : controller.reportPotentialToRenter.value
                                          .typeChart ==
                                      "hour"
                                  ? "${controller.reportPotentialToRenter.value.charts![index].time!.hour}h"
  ...
  String titleReturn(int index) {
    return index == 0 ? 'Tổng số khách tiềm năng thành người thuê' : '';
  }

  double moneyReturn(int index) {
    return controller.indexChart.value == 0
        ? ((controller.reportPotentialToRenter.value.charts ?? [])[index]
                    .totalPotentialToRenter ??
                0)
            .toDouble()
        : 0;
  }
  ...
  double moneyTitleReturn(int index) {
    return index == 0
        ? (controller.reportPotentialToRenter.value.totalPotentialToRenter ?? 0).toDouble()
        : 0;
  }
}
