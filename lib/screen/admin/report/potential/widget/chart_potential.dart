...
import '../report_potential_controller.dart';

// ignore: must_be_immutable
class PotentialChart extends StatefulWidget {
  @override
  _PotentialChartState createState() => _PotentialChartState();
}

class _PotentialChartState extends State<PotentialChart> {
  ReportPotentialController controller = Get.find();
              ...
              series: <LineSeries<SalesData, String>>[
                LineSeries<SalesData, String>(
                  ...
                  dataSource: <SalesData>[
                    ...List.generate(
                      (controller.reportPotential.value.charts ?? [])
                          .length,
                      (index) => SalesData(
                          controller.reportPotential.value.typeChart == "month" ? controller.listMonth[index] : controller.reportPotential.value.typeChart == "hour" ? "${controller.reportPotential.value.charts![index].time!.hour}h" : SahaDateUtils().getDDMM(controller.reportPotential.value.charts![index].time!),
                          moneyReturn(index)),
                    )
                  ],
  ...
  double moneyReturn(int index) {
    return controller.indexChart.value == 0
        ...
        : controller.indexChart.value == 1
            ? ((controller.reportPotential.value.charts ?? [])[index]
                        .totalPotentialNotConsultant ??
                    0)
                .toDouble()
            : controller.indexChart.value == 2
                ? ((controller.reportPotential.value.charts ?? [])[index]
                            .totalPotentialConsulting ??
                        0)
                    .toDouble()
                : controller.indexChart.value == 3
                    ? ((controller.reportPotential.value.charts ??
                                    [])[index]
                                .totalPotentialRejected ??
                            0)
                        .toDouble():
                    0;
  }
  ...
  double moneyTitleReturn(int index) {
    return index == 0
        ? (controller.reportPotential.value.totalPotentials ?? 0).toDouble()
        : index == 1
            ? (controller.reportPotential.value.totalPotentialNotConsultant ?? 0)
                .toDouble()
            : index == 2
                ? (controller.reportPotential.value.totalPotentialConsulting ??
                        0)
                    .toDouble()
                : index == 3
                    ? (controller
                                .reportPotential.value.totalPotentialRejected ??
                            0)
                        .toDouble()
                    : 0;
  }
