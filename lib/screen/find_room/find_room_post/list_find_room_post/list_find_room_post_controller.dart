...
class ListFindRoomPostController extends GetxController {
  var listRoomPost = RxList<PostFindRoom>();
  ...
  double? maxMoney;
  String? phoneNumber;
  var motelPostFilter = PostFindRoomFilter().obs;
  ...
  Future<void> getAllPostFindRoom({
    ...
    try {
      if (isEnd == false) {
        ...
        var data =
            await RepositoryManager.userManageRepository.getAllPostFindRoom(
          page: currentPage,
          status: 2,
        ...
        if (isRefresh == true) {
          listRoomPost(data!.data!.data!);
          listRoomPost.refresh();
        } else {
          listRoomPost.addAll(data!.data!.data!);
        }
